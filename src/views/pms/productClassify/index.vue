...
<script>
import { fetchList, deleteProductClassify, updateShowStatus, updateNavStatus } from '@/api/productClassify'
...
export default {
  ...
  methods: {
    ...
    handleAddProductCate () {
      this.$router.push('/pms/addProductClassify');
    },
    getList () {
    ...
    handleShowNextLevel (index, row) {
      this.$router.push({ path: '/pms/productClassify', query: { parentId: row.id } })
    },
    handleTransferProduct (index, row) {
      console.log('handleAddProductClassify');
    },
    handleUpdate (index, row) {
      this.$router.push({ path: '/pms/updateProductClassify', query: { id: row.id } });
    },
    handleDelete (index, row) {
      this.$confirm('是否要删除该品牌', '提示', {
        ...
        deleteProductClassify(row.id).then(response => {
          this.$message({
            message: '删除成功',
            type: 'success',
            duration: 1000
