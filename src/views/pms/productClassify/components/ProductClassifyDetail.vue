...
<script>
import { fetchList, createProductClassify, updateProductClassify, getProductClassify } from '@/api/productClassify';
...
export default {
  ...
  created () {
    if (this.isEdit) {
      getProductClassify(this.$route.query.id).then(response => {
        ...
        if (valid) {
          this.$confirm('是否提交数据', '提示', {
            ...
            if (this.isEdit) {
              ...
              updateProductClassify(this.$route.query.id, this.productCate).then(response => {
                this.$message({
                  message: '修改成功',
                  type: 'success',
                  duration: 1000
              ...
              this.productCate.productAttributeIdList = this.getProductAttributeIdList();
              createProductClassify(this.productCate).then(response => {
